...
import com.idreamsky.fanbook.common.ucrop.util.FBFastBitmapDrawable;
 ...
 */
public class FBTransformImageView extends AppCompatImageView implements BitmapLoadCallback {
    ...
    public FBTransformImageView(Context context) {
        this(context, null);
    }

    public FBTransformImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FBTransformImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void setScaleType(ScaleType scaleType) {
        if (scaleType == ScaleType.MATRIX) {
            ...
            Logger.e(TAG, "Invalid ScaleType. Only ScaleType.MATRIX can be used");
        }
    }
    ...
    public int getMaxBitmapSize() {
        if (mMaxBitmapSize <= 0) {
            mMaxBitmapSize = FBBitmapLoadUtils.calculateMaxBitmapSize(getContext());
        }
    ...
    public void setImageBitmap(final Bitmap bitmap) {
        mBitmapDecoded = true;
        setImageDrawable(new FBFastBitmapDrawable(bitmap));
    }
     ...
     */
    public void setImageUri(Uri inputUri, Uri outputUri, Bitmap bitmap) {
        int maxBitmapSize = getMaxBitmapSize();
        FBBitmapLoadUtils.decodeBitmapInBackground(getContext(), inputUri, outputUri, bitmap, maxBitmapSize, maxBitmapSize, this);
    }
    ...
    public void onBitmapLoaded(@NonNull Bitmap bitmap, @NonNull ExifInfo exifInfo, @NonNull Uri imageInputUri, @Nullable Uri imageOutputUri) {
        mImageInputPath = imageInputUri != null ? imageInputUri.getPath() : null;
        mImageOutputPath = imageOutputUri != null ? imageOutputUri.getPath() : null;
        mExifInfo = exifInfo;
        setImageBitmap(bitmap);
        if (mTransformImageListener != null) mTransformImageListener.onLoadComplete();
    }
    ...
    public Bitmap getViewBitmap() {
        if (getDrawable() == null || !(getDrawable() instanceof FBFastBitmapDrawable)) {
            return null;
        } else {
            return ((FBFastBitmapDrawable) getDrawable()).getBitmap();
        }
    }
    ...
    protected void onImageLaidOut() {
        ...
        Logger.d(TAG, String.format("Image size: [%d:%d]", (int) w, (int) h));

        RectF initialImageRect = new RectF(0, 0, w, h);
        mInitialImageCorners = FBRectUtils.getCornersFromRect(initialImageRect);
        mInitialImageCenter = FBRectUtils.getCenterFromRect(initialImageRect);
    ...
    protected void printMatrix(@NonNull String logPrefix, @NonNull Matrix matrix) {
        ...
        Logger.d(TAG, logPrefix + ": matrix: { x: " + x + ", y: " + y + ", scale: " + rScale + ", angle: " + rAngle + " }");
    }
