...
import com.idreamsky.fanbook.common.ucrop.util.FBCubicEasing;
import com.idreamsky.fanbook.common.ucrop.util.FBRectUtils;
 ...
 */
public class FBCropImageView extends FBTransformImageView {
    ...
    private FBCropBoundsChangeListener mCropBoundsChangeListener;
    ...
    public FBCropImageView(Context context) {
        this(context, null);
    }

    public FBCropImageView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public FBCropImageView(Context context, AttributeSet attrs, int defStyle) {
    ...
    public void cropAndSaveImage(@NonNull Bitmap.CompressFormat compressFormat, int compressQuality,
        ...
        if (imageInputPath == null || imageOutputPath == null || imageInputPath.isEmpty() || imageOutputPath.isEmpty()) {
            Logger.e("Crop image input or output path is empty!!!");
            return;
        }
        ...
        final ImageState imageState = new ImageState(
                mCropRect, FBRectUtils.trapToRect(mCurrentImageCorners),
                getCurrentScale(), getCurrentAngle());

        final CropParameters cropParameters = new CropParameters(
                mMaxResultImageSizeX, mMaxResultImageSizeY,
                compressFormat, compressQuality,
                imageInputPath, imageOutputPath, getExifInfo());
        cropParameters.setContentImageOutputUri(Uri.fromFile(new File(imageOutputPath)));
        cropParameters.setContentImageInputUri(Uri.fromFile(new File(imageInputPath)));
        File outFile = new File(imageOutputPath);
    ...
    @Nullable
    public FBCropBoundsChangeListener getCropBoundsChangeListener() {
        return mCropBoundsChangeListener;
    }

    public void setCropBoundsChangeListener(@Nullable FBCropBoundsChangeListener cropBoundsChangeListener) {
    ...
    public void setImageToWrapCropBounds(boolean animate) {
        if (mBitmapLaidOut && !isImageWrapCropBounds()) {
            ...
            if (willImageWrapCropBoundsAfterTranslate) {
                ...
                final float[] currentImageSides = FBRectUtils.getRectSidesFromCorners(mCurrentImageCorners);
            ...
            if (animate) {
                post(mWrapCropBoundsRunnable = new WrapCropBoundsRunnable(
                        FBCropImageView.this, mImageToWrapCropBoundsAnimDuration, currentX, currentY, deltaX, deltaY,
    ...
    private float[] calculateImageIndents() {
        ...
        float[] unrotatedCropBoundsCorners = FBRectUtils.getCornersFromRect(mCropRect);

        mTempMatrix.mapPoints(unrotatedImageCorners);
        mTempMatrix.mapPoints(unrotatedCropBoundsCorners);

        RectF unrotatedImageRect = FBRectUtils.trapToRect(unrotatedImageCorners);
        RectF unrotatedCropRect = FBRectUtils.trapToRect(unrotatedCropBoundsCorners);
    ...
    protected boolean isImageWrapCropBounds(float[] imageCorners) {
        ...
        return FBRectUtils.trapToRect(unrotatedImageCorners).contains(FBRectUtils.trapToRect(unrotatedCropBoundsCorners));
    }
    ...
    protected void zoomImageToPosition(float scale, float centerX, float centerY, long durationMs) {
        ...
        post(mZoomImageToPositionRunnable = new ZoomImageToPosition(FBCropImageView.this,
    ...
    private static class WrapCropBoundsRunnable implements Runnable {
        ...
        private final WeakReference<FBCropImageView> mCropImageView;
        ...
        public WrapCropBoundsRunnable(FBCropImageView cropImageView,
                                      long durationMs,
        ...
        @Override
        public void run() {
            FBCropImageView cropImageView = mCropImageView.get();
            ...
            float newX = FBCubicEasing.easeOut(currentMs, 0, mCenterDiffX, mDurationMs);
            float newY = FBCubicEasing.easeOut(currentMs, 0, mCenterDiffY, mDurationMs);
            float newScale = FBCubicEasing.easeInOut(currentMs, 0, mDeltaScale, mDurationMs);
    ...
    private static class ZoomImageToPosition implements Runnable {
        ...
        public ZoomImageToPosition(FBCropImageView cropImageView,
                                   long durationMs,
                                   float oldScale, float deltaScale,
