...
class PreviewCollectionViewCell: UICollectionViewCell, UIGestureRecognizerDelegate, UIScrollViewDelegate {
    var photoImageView : FLAnimatedImageView = FLAnimatedImageView(frame: CGRect.zero)
    ...
    var gifImage : FLAnimatedImage? {
        didSet{
            if gifImage != nil {
                photoImageView.image = gifImage?.posterImage;
                photoImageView.sizeToFit()
                photoImageView.animatedImage = gifImage
                
                let scaleW = scrollView.frame.width/gifImage!.size.width
                let scaleH = scrollView.frame.height/gifImage!.size.height
                let scale = min(min(scaleW, scaleH), 1.0)
                scrollView.minimumZoomScale = scale
                scrollView.maximumZoomScale = scale * 10
                scrollView.zoomScale = scale
            ...
            if asset != nil {
                ...
                self.tag = Int(PHCachingImageManager.default().requestImageData(for: asset!, options: options) { (data, uti, orientation, info) in
                    guard let tUti = uti, let result = data else {
                        ...
                        weakSelf?.image = UIImage.tz_imageNamed(fromMyBundle: "ic_photo_error")
                        return
                    }
                    weakSelf?.thumbCanLoad = true
                    if (tUti.contains("gif")) {
                        weakSelf?.gifImage = FLAnimatedImage(animatedGIFData: result)
                    }else {
    ...
    override init(frame: CGRect) {
        ...
        playImageView.image = UIImage.tz_imageNamed(fromMyBundle: "multi_play_btn_unselect")
        ...
        let tapGesture = UITapGestureRecognizer(target: self, action: #selector(cellDidReceiveTapAction))
    ...
    @objc func cellDidReceiveTapAction() {
        ...
        self.delegate?.previewCellDidReceiveTapAction(self)
    }
