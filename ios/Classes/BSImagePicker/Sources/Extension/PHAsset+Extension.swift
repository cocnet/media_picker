...
//最大像素
let MAXPixel:CGFloat = 20000000.0
...
public extension PHAsset {
    ...
    func factDuration( calculation:@escaping (Double) -> Void)  {
        if mediaType == PHAssetMediaType.video &&
            mediaSubtypes == PHAssetMediaSubtype.videoHighFrameRate{
            
            let requestOptions = PHVideoRequestOptions()
            requestOptions.version = .current
            PHCachingImageManager().requestAVAsset(forVideo: self,
    ...
    @objc public func compressAsset(_ thumb : Bool, saveDir : String, process: ((NSDictionary) -> Void)?, failed: ((NSError) -> Void)?, finish: ((NSDictionary) -> Void)?) {
                        ...
                        DispatchQueue.global().async {
                            if let imageData = (thumb ? UIImage.lubanCompressImage(image) : UIImage.lubanOriginImage(image)) as NSData? {
                                if thumb {
                                    ...
                                    if targetWidth * targetHeight > CHECKIMAGESIZE * CHECKIMAGESIZE, let checkImage = UIImage.compressImage(UIImage(data: imageData as Data), toTargetWidth: CHECKIMAGESIZE, toTargetWidth: CHECKIMAGESIZE), let checkImageData = checkImage.jpegData(compressionQuality: 1.0) as NSData? {
