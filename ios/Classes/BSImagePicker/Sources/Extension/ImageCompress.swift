...
public extension ImageCompress {
    ...
    针对gif的尺寸进行压缩
     */
    static let minSizeConstraint:CGFloat = 48.0;
    static let maxSizeConstraint:CGFloat = 225.0;
    static func resetGifOriginalSize(width: CGFloat, height: CGFloat) -> CGSize{
        ...
        let devicePixelRatio:CGFloat = 2.0; //屏幕密度
        ...
        //对于尺寸很小的gif, *devicePixelRatio 计算之后可能导致反而变大, 就直接返回
        if(width < targetWidth * devicePixelRatio && height <  targetHeight * devicePixelRatio){
