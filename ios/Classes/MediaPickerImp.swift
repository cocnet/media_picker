...
class MediaPickerImp{
    public class func handleMethodCall(_ call: FlutterMethodCall, result: @escaping FlutterResult) {
        ...
        case "loadAlbums":
            result(DataCenter.shared.fetchPhotoAlbumInfoList())
        ...
        case "fetchMediaInfo":
            ...
            let albumId = (arguments["albumId"] as? Int)
            let expectType = (arguments["mediaShowTypes"] as? String) ?? "all"
            let selectedAssets = (arguments["selectedAssets"] as? Array<String>) ?? []
            ...
            DispatchQueue.global().async {
                ...
                if selectedAssets.count <= 0 {
                    if albumId == nil {
                        let fetchCollectionOptions = PHFetchOptions()
                        let fetchAssetOptions = PHFetchOptions()
                        let cameraRollResult = PHAssetCollection.fetchAssetCollections(with: .smartAlbum, subtype: .smartAlbumUserLibrary, options: fetchCollectionOptions)
                        if cameraRollResult.firstObject != nil {
                            collection = cameraRollResult.firstObject!
                        }
                    }else{
                        let collections = DataCenter.shared.fetchResults
                        if(collections.isEmpty || albumId! >= collections.count){
                            result([])
                            return
                        }
                        collection = collections[albumId!]
                    }
                }
                ...
                }else if expectType == "video" {
                    fetchAssetOptions.predicate = NSPredicate(format: "mediaType = %d", PHAssetMediaType.video.rawValue)
                }
                let fetchResult = selectedAssets.count > 0 ? PHAsset.fetchAssets(withLocalIdentifiers: selectedAssets, options: nil) :
                    (collection != nil ? PHAsset.fetchAssets(in: collection!, options: fetchAssetOptions) : PHAsset.fetchAssets(with: fetchAssetOptions))
...
//                fetchResult不能倒序，所以在for循环的时候倒序
                ...
                if from >= 0, from >= through {
                    for i in stride(from: from, through:max(through, 0), by:-1)  {
                        ...
                        let size = DataCenter.shared.getThumbnailSize(originSize: CGSize(width: asset.pixelWidth, height: asset.pixelHeight))
                        ...
                        if asset.mediaType == .video {
                            ...
                            if asset.mediaSubtypes == PHAssetMediaSubtype.videoHighFrameRate{
                                ...
                                group.enter()
                                asset.factDuration { dur in
                                    dictionary.setValue(dur, forKey: "duration")
                                    group.leave()
                                }
                                group.wait()
                            }
        ...
        case "fetchMediaThumbData":
            let arguments = call.arguments as! Dictionary<String, AnyObject>
            let localIdentifier = (arguments["identifier"] as? String) ?? ""
            let thumb = (arguments["thumb"] as? String) ?? "" == "thumb"
            DispatchQueue.global().async {
                ...
                if let asset = PHAsset.fetchAssets(withLocalIdentifiers: [localIdentifier], options: nil).firstObject {
                    let originSize = CGSize(width: CGFloat(asset.pixelWidth), height: CGFloat(asset.pixelHeight))
                    PHCachingImageManager.default().requestImage(for: asset, targetSize: thumb ? DataCenter.shared.getThumbnailSize(originSize: originSize) : originSize, contentMode: imageContentMode, options: imageRequestOptions) { (image, info) in
        ...
        case "requestFilePath":
            ...
            DispatchQueue.global().async {
                if let asset = PHAsset.fetchAssets(withLocalIdentifiers: [localIdentifier], options: nil).firstObject {
                    let fileDownloadPath = DataCenter.shared.downloadiCloudMediaPath(originName: asset.originalFilename ?? "")
                    if FileManager.default.fileExists(atPath: fileDownloadPath) {
                        result(["filePath" : fileDownloadPath])
                    }else {
        ...
        case "pickImages":
            ...
            let defaultIndex = (arguments["albumId"] as? Int) ?? 0
            let selectedAssets = (arguments["selectedAssets"] as? Array<String>) ?? [];
            ...
            let mediaCanDownloadFromiCloud = Int((options["mediaCanDownloadFromiCloud"] as? String) ?? "0")
            let videoCanPickLimitSize = Int((options["videoCanPickLimitSize"] as? String) ?? "0")
            let videoCanPickLimitDuration = Int((options["videoCanPickLimitDuration"] as? String) ?? "0")
            let imageCanPickLimitSize = Int((options["imageCanPickLimitSize"] as? String) ?? "0")
            let imageCanPickLimitPixel = Int((options["imageCanPickLimitPixel"] as? String) ?? "0")
            let imageCanPickLimitSide = Int((options["imageCanPickLimitSide"] as? String) ?? "0")
            ...
            settings.mediaCanDownloadFromiCloud = mediaCanDownloadFromiCloud == 1
            settings.videoCanPickLimitSize = videoCanPickLimitSize!
            settings.videoCanPickLimitDuration = videoCanPickLimitDuration!
            settings.imageCanPickLimitSize = imageCanPickLimitSize!
            settings.imageCanPickLimitPixel = imageCanPickLimitPixel!
            settings.imageCanPickLimitSide = imageCanPickLimitSide!
            ...
            DataCenter.shared.defaultSelectMediaAlbumId = defaultIndex
...
//            let url = (arguments["url"] as? String) ?? ""
        ...
        case "deleteCacheVideo":
            ...
            let exist = fileManager.fileExists(atPath: cacheDir ?? "")
            if (exist) {
