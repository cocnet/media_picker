...
@property(nonatomic,strong) UILabel *icloudLable;
@end
...
- (void)setModel:(PHAsset *)model {
    ...
    int32_t imageRequestID = [[TZImageManager manager] getPhotoWithAsset:model photoWidth:self.tz_width completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
...
//    self.selectPhotoButton.selected = model.isSelected;
    ...
    TZImagePickerController *tzImagePickerVc = (TZImagePickerController*)[self findViewController].navigationController;
    NSInteger i = [tzImagePickerVc.selectedAssetIds indexOfObject:self.model.localIdentifier];
    if(tzImagePickerVc.showSelectedIndex && i != NSNotFound){
    ...
    self.type = [[TZImageManager manager] getAssetType:model];
...
//    }
    ...
    if (self.assetCellDidSetModelBlock) {
        self.assetCellDidSetModelBlock(self, _imageView, _selectImageView, _indexLabel, _bottomView, _timeLength, nil);
    }
...
//    self.icloudLable.text = bIsLocallayAvailable ? @"":@"icloud";
}
...
- (void)setShowSelectBtn:(BOOL)showSelectBtn {
    ...
    BOOL selectable = YES;//[[TZImageManager manager] isPhotoSelectableWithAsset:self.model.asset];
...
//        self.timeLength.text = timeLength;
        self.timeLength.text = [[TZImageManager manager] getNewTimeFromDurationSecond:self.model.duration];
...
//    BOOL notSelectable = [TZCommonTools isAssetNotSelectable:self.model tzImagePickerVc:tzImagePickerVc];
    BOOL notSelectable = tzImagePickerVc.selectedAssetIds.count >= tzImagePickerVc.maxImagesCount;
    if (notSelectable && tzImagePickerVc.showPhotoCannotSelectLayer && (i == NSNotFound)) {
...
- (UIView *)bottomView {
    if (_bottomView == nil) {
        ...
        bottomView.backgroundColor = [UIColor clearColor];// [UIColor colorWithRed:rgb green:rgb blue:rgb alpha:0.8];
...
- (UILabel *)timeLength {
    if (_timeLength == nil) {
        ...
        timeLength.backgroundColor = [UIColor nv_colorWithHexString:@"#1F2126" alpha:0.55];
        timeLength.layer.cornerRadius = 2;
        [timeLength.layer setMasksToBounds:YES];
...
- (UILabel *)icloudLable {
    if (_icloudLable == nil) {
        UILabel *indexLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, 0, 40, 30)];
        indexLabel.font = [UIFont systemFontOfSize:14];
        indexLabel.adjustsFontSizeToFitWidth = YES;
        indexLabel.textColor = [UIColor whiteColor];
        indexLabel.textAlignment = NSTextAlignmentCenter;
        [self.contentView addSubview:indexLabel];
        _icloudLable = indexLabel;
    }
    return _icloudLable;
}
...
- (void)layoutSubviews {
    ...
    _bottomView.frame = CGRectMake(0, self.tz_height - 17 - 4, self.tz_width, 17);
...
//    _timeLength.frame = CGRectMake(self.videoImgView.tz_right, 0, self.tz_width - self.videoImgView.tz_right - 5, 17);
    _timeLength.frame = CGRectMake(self.tz_width-36-4, 0, 36, 16);
    
    self.type = [[TZImageManager manager] getAssetType:self.model];// (NSInteger)self.model.type;
//    model.
    ...
    if (self.assetCellDidLayoutSubviewsBlock) {
        self.assetCellDidLayoutSubviewsBlock(self, _imageView, _selectImageView, _indexLabel, _bottomView, _timeLength, nil);
    }
}
