...
-(void)configDefalutPicker{
    ...
    self.isSelectOriginalPhoto = true;
    self.allowCrop = false;
    self.allowPickingImage = true;
    self.allowPickingVideo = true;
    self.allowEditVideo = false;
    self.allowTakeVideo = false;
    self.allowTakePicture = false;
    self.allowPreview = true;
    self.showPhotoCannotSelectLayer = true;
    self.cannotSelectLayerColor = [UIColor colorWithRed:1.0 green:1.0 blue:1.0 alpha:0.5f];
    self.showSelectedIndex = true;
    self.allowPickingOriginalPhoto = false; //默认不使用原图
    self.pickMode = TZPickAssetsForEdit;
    self.selectedModels = [[NSMutableArray alloc] init];
    self.selectedAssetIds = [[NSMutableArray alloc] init];
    self.timeout = 30;
    self.photoSelImage = [UIImage tz_imageNamedFromMyBundle:@"asset_checked_icon"];
    self.photoDefImage =  [UIImage tz_imageNamedFromMyBundle:@"asset_uncheck_icon"];
    self.sortAscendingByModificationDate=true;
    self.statusBarStyle = UIStatusBarStyleLightContent;
    self.modalPresentationStyle = UIModalPresentationFullScreen;
    
    //优化gif播放
    [TZImagePickerConfig sharedInstance].gifPreviewMaxImagesCount = 100;
...
//    [FBIQKeyboradTool changeIQKeyboradToolSatusWithIsOpen:YES];
    [[FBMeisheSourceTool sharedInstance] updateNetWorkSource];
...
- (void)configNavigationBarAppearance {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        if (self.navigationBar.isTranslucent) {
            ...
            barAppearance.backgroundColor = [UIColor blackColor];//[barTintColor colorWithAlphaComponent:0.85];
        } else {
            barAppearance.backgroundColor = [UIColor nv_colorWithHexRGB:@"#141414"];// self.navigationBar.barTintColor;
        }
        ...
        self.navigationBar.backgroundColor =  [UIColor nv_colorWithHexRGB:@"#141414"];
    }
}
...
- (void)dealloc {
//    [FBIQKeyboradTool changeIQKeyboradToolSatusWithIsOpen:NO];
    // NSLog(@"%@ dealloc",NSStringFromClass(self.class));
}

- (void)addSelectedModel:(PHAsset *)model {
    [_selectedModels addObject:model];
    [_selectedAssetIds addObject:model.localIdentifier];
}

- (void)removeSelectedModel:(PHAsset *)model {
