...
typedef NS_ENUM(NSUInteger, TZPickAssetsMode) {
    TZPickAssetsForEdit,
    TZPickAssetsForAdd,
};
...
#pragma mark -
/// 打开选择器，选择模式
@property (nonatomic, assign) TZPickAssetsMode pickMode;
...
@property (nonatomic, strong) NSMutableArray<PHAsset *> *selectedModels;
...
@property (nonatomic, copy) void (^didFinishPickingAssetsHandle)(NSArray *asstes);

/// 美摄sdk 视频， 图片处理完成回调 id 类型到时候再定
@property (nonatomic, copy) void (^didFinishMediaHandle)(NSArray<id> *asstes, NSError *error);

/// 用户取消
@property (nonatomic, copy) void (^didCancelMediaHandle)();

/// 生成封面成功
@property (nonatomic, copy) void (^didFinishGenerateVideoCoverHandle)(id asstes, NSError *error);
