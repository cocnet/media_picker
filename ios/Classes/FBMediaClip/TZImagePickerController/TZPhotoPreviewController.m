...
@property(nonatomic,strong) HWThrottle *clickThrottler;
@end
...
- (void)configCustomNaviBar {
    ...
    [_backButton setImage:[UIImage tz_imageNamedFromMyBundle:@"nav_back_btn"] forState:UIControlStateNormal];
    [_backButton setImageEdgeInsets:UIEdgeInsetsMake(10, 10, 10, 10)];
    [_backButton setTitleColor:[UIColor whiteColor] forState:UIControlStateNormal];
    [_backButton addTarget:self action:@selector(backButtonClick) forControlEvents:UIControlEventTouchUpInside];
    
    _selectButton = [[UIButton alloc] initWithFrame:CGRectZero];
    [_selectButton setImage:[UIImage tz_imageNamedFromMyBundle:@"nav_res_uncheck"] forState:UIControlStateNormal];
    [_selectButton setImage:[UIImage tz_imageNamedFromMyBundle:@"nav_res_checked"] forState:UIControlStateSelected];
    [_selectButton setTitle:@"选择" forState:UIControlStateNormal];
    _selectButton.titleLabel.font = [UIFont systemFontOfSize:16];
    _selectButton.imageEdgeInsets = UIEdgeInsetsMake(11, 13, 11, -13);
    _selectButton.titleEdgeInsets = UIEdgeInsetsMake(1, -20, -1, 0);
    _selectButton.imageView.clipsToBounds = YES;
    [_selectButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentRight];
    ...
    _indexLabel.textColor = [UIColor clearColor];
...
- (void)viewDidLayoutSubviews {
    ...
    _selectButton.frame = CGRectMake(self.view.tz_width - 56 - 60, 10 + statusBarHeightInterval, 44+60, 44);
...
- (void)select:(UIButton *)selectButton {
    if (!self.clickThrottler) {
        @weakify(self);
        self.clickThrottler = [[HWThrottle alloc] initWithInterval:0.2 taskBlock:^{
            @strongify(self);
            [self select:selectButton refreshCount:YES];
        }];
    }
    [self.clickThrottler call];
}

- (void)select:(UIButton *)selectButton refreshCount:(BOOL)refreshCount {
    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    PHAsset *model = _models[self.currentIndex];
    if (!selectButton.isSelected) {
        // 1. select:check if over the maxImagesCount / 选择照片,检查是否超过了最大个数的限制
        if (_tzImagePickerVc.selectedModels.count >= _tzImagePickerVc.maxImagesCount) {
            NSString *title = [NSString stringWithFormat:@"最多只能选择%zd个文件", _tzImagePickerVc.maxImagesCount];
            [JYToast show:title];
            ...
            if ([[TZImageManager manager] isAssetCannotBeSelected:model]) {
                return;
            }
...
//            }
        ...
        for (PHAsset *model_item in selectedModels) {
            if ([model.localIdentifier isEqualToString:model_item.localIdentifier]) {
                ...
                for (NSInteger i = 0; i < selectedModelsTmp.count; i++) {
                    PHAsset *model = selectedModelsTmp[i];
...
//    model.isSelected = !selectButton.isSelected;
    ...
    if (selectButton.isSelected) {
        [self performSelector:@selector(backButtonClick) withObject:nil afterDelay:.15];
    }
}
...
- (void)doneButtonClick {
    ...
    if (_tzImagePickerVc.allowCrop && [cell isKindOfClass:[TZPhotoPreviewCell class]]) { // 裁剪状态
        ...
        if (self.doneButtonClickBlockCropMode) {
            PHAsset *model = _models[self.currentIndex];
            self.doneButtonClickBlockCropMode(cropedImage,model);
        }
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath {
    TZImagePickerController *_tzImagePickerVc = (TZImagePickerController *)self.navigationController;
    PHAsset *model = _models[indexPath.item];
    TZAssetModelMediaType type = [[TZImageManager manager] getAssetType:model];
    TZAssetPreviewCell *cell;
    __weak typeof(self) weakSelf = self;
    if (_tzImagePickerVc.allowPickingMultipleVideo && type == TZAssetModelMediaTypeVideo) {
...
//            model.iCloudFailed = isSyncFailed;
    ...
    } else if (_tzImagePickerVc.allowPickingMultipleVideo && type == TZAssetModelMediaTypePhotoGif && _tzImagePickerVc.allowPickingGif) {
...
//            model.iCloudFailed = isSyncFailed;
        ...
        NSString *reuseId = type == TZAssetModelMediaTypePhotoGif ? @"TZPhotoPreviewCellGIF" : @"TZPhotoPreviewCell";
...
- (void)dealloc {
    [self.clickThrottler invalidate];
...
- (void)refreshNaviBarAndBottomBarState {
    ...
    NSInteger i = [_tzImagePickerVc.selectedAssetIds indexOfObject:model.localIdentifier];

    _selectButton.selected = i != NSNotFound;
    ...
    if (![[TZImageManager manager] isPhotoSelectableWithAsset:model]) {
...
- (void)didICloudSyncStatusChanged:(PHAsset *)model{
...
//        PHAsset *currentModel = self.models[self.currentIndex];
