...
//        _imageView.backgroundColor = [UIColor colorWithWhite:1.000 alpha:0.500];
        ...
        _iCloudErrorIcon.image = [UIImage tz_imageNamedFromMyBundle:@"ic_photo_error"];
...
- (void)setModel:(PHAsset *)model {
    ...
    if (type == TZAssetModelMediaTypePhotoGif) {
        // 先显示缩略图
        [[TZImageManager manager] getPhotoWithAsset:model completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            if (photo) {
            ...
            [[TZImageManager manager] getOriginalPhotoDataWithAsset:model progressHandler:^(double progress, NSError *error, BOOL *stop, NSDictionary *info) {
                ...
                dispatch_async(dispatch_get_main_queue(), ^{
                    ...
                    if (self.iCloudSyncFailedHandle) {
                        self.iCloudSyncFailedHandle(model, iCloudSyncFailed);
                    }
...
- (void)configMaximumZoomScale {
    _scrollView.maximumZoomScale = _allowCrop ? 6.0 : 6.0;
...
//    [_playButton setImage:[UIImage tz_imageNamedFromMyBundle:@"MMVideoPreviewPlayHL"] forState:UIControlStateHighlighted];
    ...
    _playButton.imageView.width = 60;
    _playButton.imageView.height = 60;
...
- (void)configMoviePlayer {
    ...
    if (self.model) {
        [[TZImageManager manager] getPhotoWithAsset:self.model completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
            BOOL iCloudSyncFailed = !photo && [TZCommonTools isICloudSyncError:info[PHImageErrorKey]];
            self.iCloudErrorLabel.hidden = !iCloudSyncFailed;
            self.iCloudErrorIcon.hidden = !iCloudSyncFailed;
            if (self.iCloudSyncFailedHandle) {
                self.iCloudSyncFailedHandle(self.model, iCloudSyncFailed);
            }
            if (photo) {
                self.cover = photo;
            }
        }];
        [[TZImageManager manager] getVideoWithAsset:self.model completion:^(AVPlayerItem *playerItem, NSDictionary *info) {
            dispatch_async(dispatch_get_main_queue(), ^{
