...
-(void)requestAssetFileSize:(NSString*)localIdentifier complete:(void (^)(int fileSize))complete{
    PHAsset *mediaAsset = [PHAsset fetchAssetsWithBurstIdentifier:localIdentifier options:nil].firstObject;
    if(mediaAsset == nil){
        complete(0);
        return;
    }
...
//         }}];
        PHAssetResource *resource = [[PHAssetResource assetResourcesForAsset:mediaAsset] firstObject];
        long long originFileSize = [[resource valueForKey:@"fileSize"] longLongValue];
        complete(originFileSize);
        return;
    }else if(mediaAsset.mediaType == PHAssetMediaTypeImage){
        [[PHImageManager defaultManager] requestImageDataForAsset:mediaAsset options:nil resultHandler:^(NSData *imageData, NSString *dataUTI, UIImageOrientation orientation, NSDictionary *info) {
...
- (BOOL)isAssetCannotBeSelected:(PHAsset *)asset {
    ...
    if(asset.mediaType == PHAssetMediaTypeImage){
        double px = asset.pixelWidth * asset.pixelHeight;
        if(asset.pixelWidth > 12000 || asset.pixelHeight > 12000){
            [JYToast show:@"图片过大，暂不支持"];
            return YES;
        }
        if(px > self.imageCanPickLimitPixel){
            [JYToast show:@"图片过大，暂不支持"];
            return YES;
        }
        PHAssetResource *resource = [[PHAssetResource assetResourcesForAsset:asset] firstObject];
        long long originFileSize = [[resource valueForKey:@"fileSize"] longLongValue];
        if(originFileSize > self.imageCanPickLimitSize){
            [JYToast show:@"图片过大，暂不支持"];
            return YES;
        }
    }
    ...
    if(asset.mediaType == PHAssetMediaTypeVideo && self.videoCanPickLimitSize != 0){
        PHAssetResource *resource = [[PHAssetResource assetResourcesForAsset:asset] firstObject];
        long long originFileSize = [[resource valueForKey:@"fileSize"] longLongValue];
        if(originFileSize > self.videoCanPickLimitSize){
            [JYToast show:[TZImageManager genMaxSizeTip:self.videoCanPickLimitSize]];
            return YES;
        }
    }
    return NO;
...
+ (NSString *)genMaxSizeTip:(NSInteger)maxSize{
    
    int size =  maxSize / 1024 / 1024;
    
    NSString *sizeDesc = @"";
    if(size > 1024){
        sizeDesc = [NSString stringWithFormat:@"%iGB",size / 1024];
    }else{
        sizeDesc =[NSString stringWithFormat:@"%iMB",size];
    }
    return [NSString stringWithFormat:@"发送的视频文件大小不能大于%@",sizeDesc];
}
